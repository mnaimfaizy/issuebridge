...
- Sign in state now follows GitHub, not just the credential vault: Issuebridge validates the stored token on launch and signs you out (routing back to Sign in) when GitHub rejects it, instead of showing the Inbox while every request fails. A still-valid token keeps you signed in across a machine restart, and being offline no longer signs you out. (#111)
