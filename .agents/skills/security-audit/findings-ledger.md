...
| `audit-agent-unscoped-read-tools` | Audit agent allowlist grants unscoped filesystem reads | `.github/workflows/claude-security-audit.yml` | medium | untriaged | open | GHSA-qhpf-hfrv-p8pm | 2026-09-04 |
...
- `audit-agent-unscoped-read-tools`: **untriaged** — the audit allowlist still carries the path-unrestricted read rules removed from the two sibling workflows, in both modes. Same class as `agent-search-bash-unscoped-read`; the deny-by-default contract added there does not cover this workflow's allowlist.
