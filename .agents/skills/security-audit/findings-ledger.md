...
| `security-audit-persisted-checkout-credentials` | Audit workflow checkout persists the job token into the agent workspace | `.github/workflows/claude-security-audit.yml` | high | untriaged | open | GHSA-qhpf-hfrv-p8pm | 2026-09-04 |
...
- `security-audit-persisted-checkout-credentials`: **untriaged** — the audit workflow checkout does not set the control both sibling agent workflows set, and no contract test asserts it there. Severity contested between the two advisories (medium on the earlier, high on the later); filed at the higher pending a maintainer decision. Distinct from `agent-pipeline-persisted-checkout-credentials` (pipeline workflow), which covers a different workflow and holds.
